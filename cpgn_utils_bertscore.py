...
def bert_indexify_transformations(in_p, out_p, label_voc, args):
    ...
    # otherwise, indexify and return
        ...
        # print 'not none'
        in_trans_np = np.zeros((len(in_seqs), max_trans_size), dtype='float32')
        out_trans_np = np.zeros((len(in_seqs), max_trans_size), dtype='float32')
        ...
        for idx in range(len(in_seqs)):
            ...
            # print [label_voc[z] for z in curr_in]
            # input('inp')
...
def create_trans_emb_weights(label_vocab, max_trans_size):
    ...
    for word in label_vocab:
        try:
            ...
            # print 'label_vocab[word] : {}'.format(label_vocab[word])
            # print '[1]*word : {}'.format([1]*int(word))
            weights_matrix[label_vocab[word], :int(word)] = [1]*int(word)
            # print weights_matrix[label_vocab[word], ]
            # input('input: ')
...
def get_list_of_unique_berts(): #TODO beram min max ro peyda konam scale koname ?????????
...
def parse_bertscore():
    print ('parse bert scores')
    index_scores = {}

    # (0.6618, 1.0) for new_all
    min_score = 0.6618
    max_score = 1.0
    with open('bertscore_similarities_new_all.txt') as f:
    # with open('bertscore_similarities_new3.txt') as f:
        for line in f.readlines():
            splits = line.split('\t')
            idx, scores = splits[0], splits[1]
            # print scores
            scores = scores[1:-1]
            answer_strip= scores.split(', ')[2][:6].strip()
            if answer_strip[-1] == ')':
                answer_strip = answer_strip[:-1]
            # print(repr(answer_strip))
            f1 =float(answer_strip)
            idx = int(idx[:idx.index('_')])

            f1_norm = (f1 - min_score) / (max_score - min_score)
            new_f1 = int(round(f1_norm*40))
            print ('f1: {} \t f1_norm: {} \t new_f1: {}'.format(f1, f1_norm, new_f1))
            index_scores[idx] = new_f1

    print ('min_score, max_score: ')
    print (min_score, max_score)

    f = open('index_scores.pkl', 'wb')
    cPickle.dump(index_scores, f)
    f.close()
