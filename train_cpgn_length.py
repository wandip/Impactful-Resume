...
from cpgn_utils_bertscore import *
...
class CPGN(nn.Module):
    def __init__(self, d_word, d_hid, d_nt, d_trans,
        len_voc, len_trans_voc, use_input_parse, label_vocab):
    ...
    def decode_step(self, idx, prev_words, prev_hid, prev_cell, 
        ...
        # print 'prev_words: {}'.format(prev_words) #only last word for each input [] of len 62
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Syntactically Controlled Paraphrase Network: Bertscore')
    ...
    parser.add_argument('--parse_vocab', type=str, default='data/unique_lenght.txt',
            help='tag vocabulary')
    parser.add_argument('--model', type=str, default='CPGN2_bert_length_4k_temp.pt',
    ...
    parser.add_argument('--d_nt', type=int, default=56,
            help='nonterminal embedding dimension')#length embedding dimension
    parser.add_argument('--d_hid', type=int, default=512,
            help='lstm hidden dimension')
    parser.add_argument('--n_epochs', type=int, default=5, #TODO changed from 15 to 5 accroding to git issues
            help='n_epochs')
    ...
    for idx, line in enumerate(tag_file):
        ...
        # if line != 'EOP':
        label_voc[line] = idx
    rev_label_voc = dict((v,k) for (k,v) in label_voc.items()) 
    ...
    with open('minibatches/test_minibatches_' + args.model + '.txt', 'w') as f:
        for b_idx, (start, end) in enumerate(test_minibatches):
            f.write('{},{}'.format(start, end))
            f.write('\n')

    # build network
    net = CPGN(d_word, d_hid, d_nt, d_trans,
        len_voc, len_parse_voc, args.use_input_parse, label_voc)
    net.cuda()
    ...
    if args.eval_mode:
        ...
        train_minibatches = saved_model['train_minibatches']
    ...
    for ep in range(n_epochs):
        ...
        for b_idx, (start, end) in enumerate(train_minibatches):
            ...
            in_p = in_lens[start:end]
            out_p = out_lens[start:end]

            in_p = [str(i) for i in in_p]
            out_p = [str(i) for i in out_p]

            z = bert_indexify_transformations(in_p, out_p, label_voc, args)
            if z==None:
                continue
            ...
            print('in_len_np: {}'.format(in_len_np.shape))
            print('out_len_np: {}'.format(out_len_np.shape))
            print('in_trans_len_np: {}'.format(in_trans_len_np.shape))
            print('out_trans_len_np: {}'.format(out_trans_len_np.shape))
                ...
                for i in range(min(3, curr_bsz)):
                    ...
                    try:
                        ...
                        print ('gt output: %s' % ' '.join([rev_vocab[w] for (j,w) in enumerate(out_np[i, :eos])\
                            if j < out_len_np[i]]))
                        print ('length: %s' % ' '.join([rev_label_voc[z] for (j,z) in enumerate(np_o)\
